...
data_train_day[[1]]$weekdays <- unlist(lapply(X = data_train_day[[1]]$weekdays, FUN = daysToNumber))
data_test_day[[1]]$weekdays <- unlist(lapply(X = data_test_day[[1]]$weekdays, FUN = daysToNumber))

# fix seed
set.seed(1)

# convert dataframes to xgb.DMatrix
data_train_day_ = xgb.DMatrix(as.matrix(data_train_day[[1]] %>% select(-rateCar)))
rateCar_train = data_train_day[[1]]$rateCar
data_test_day_ = xgb.DMatrix(as.matrix(data_test_day[[1]] %>% select(-rateCar)))
rateCar_test = data_test_day[[1]]$rateCar
...
ptm <- proc.time()
xgb_model = caret::train(data_train_day[[1]], data_train_day[[1]]$rateCar, trControl = xgb_trcontrol, tuneGrid = xgbGrid, method = "xgbTree")
print(proc.time() - ptm) 
...
params <- list(booster = "gbtree",
               eta=0.01, gamma=0.4, max_depth=7, 
               min_child_weight=1, subsample=0.9, 
               colsample_bytree=0.9)

ptm <- proc.time()
xgbcv <- xgb.cv(params = params, 
                data = as.matrix(data_train_day[[1]] %>% select(-rateCar)), 
                label = data_train_day[[1]]$rateCar,
                nrounds = 5000, nfold = 5, showsd = T, 
                print_every_n = 10, maximize = F,
                metrics = "rmse", early_stopping_rounds=5)
print(proc.time() - ptm) 

# building model
model <- xgboost(data = as.matrix(data_train_day[[1]] %>% select(-rateCar)), 
                 label = data_train_day[[1]]$rateCar,
                 nrounds = 805,
                 booster = "gbtree",
                 metrics = "rmse",
...
pred <- predict(model, as.matrix(data_test_day[[1]] %>% select(-rateCar)))

rmse(pred,data_test_day[[1]]$rateCar)
...
data_train_day <- readRDS("C:/Users/lambe/Documents/Projet_ML/données/train_one_model.rds")
data_test_day <- readRDS("C:/Users/lambe/Documents/Projet_ML/données/test_one_model.rds")

model <- xgboost(data = as.matrix(data_train_day %>% select(-nexthour_rateCar)), 
                 label = data_train_day$nexthour_rateCar,
                 nrounds = 133,
                 booster = "gbtree",
                 metrics = "rmse",
                 eta=0.1, gamma=0.2, max_depth=7, 
