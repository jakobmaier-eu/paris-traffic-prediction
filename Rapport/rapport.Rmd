...
```{r now_vs_onehour, echo = F, cache=TRUE}
...
```{r linReg_heureclustering, echo = F, cache=TRUE}
...
cluster_hours = function(h){
  if (h %in% c(0,1,2,3,4,5,6)){return("night")}
...
1. **hour** : Circulaire, essayons différents k.
...
# Modèles - TODO Jakob: Relire et intégrer à partir d'ici
...
df_box <- data.frame("100", "3, 5, 7, 10",
                               "0.70, 0.75, 0.80, 0.85, 0.90",
                               "0.1","0.2","1,3,5,7,9",
                               "0.70, 0.75, 0.80, 0.85, 0.90")
names(df_xgboostTuning) <- c("nrounds", "max_depth",
